...
| `BBB_AZURE_FORCE_S2S` | *(off)* | Set to `1` or `true` to force Azure→Azure server-side (S2S) copy. On a server-side copy failure, bbb does **not** fall back to client-side streaming; instead the error is retried (honouring `--retry-count`) |
